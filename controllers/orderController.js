...
const getLatLng = async (toCheckAddress) => {
  ...
  if (!address || !city || !pin_code || !state) {
    return res
      .status(400)
      .json({ success: false, message: "Incomplete address information" });
  }
  ...
  try {
    ...
    console.error("Error occured while get lat lng", error);
...
async function SendOrderConfirmMailAfterSomething(orderId, session) {
  ...
  try {
    ...
    const requiredOrder = await Order.findById(
      mongoose.Types.ObjectId(orderId)
    ).session(session);
    ...
    const shippingAddress = [
      requiredOrder.shippingInfo.deliveryAddress.address,
      requiredOrder.shippingInfo.deliveryAddress.locality,
      requiredOrder.shippingInfo.deliveryAddress.landmark,
      requiredOrder.shippingInfo.deliveryAddress.city,
      requiredOrder.shippingInfo.deliveryAddress.pin_code,
      requiredOrder.shippingInfo.deliveryAddress.state,
    ]
      .filter(Boolean)
      .join(", ");
    const items = requiredOrder.orderItems || [];
    const subject = "Order placed at Fresh Vegie for " + requiredOrder.orderId;
    const createdAtUTC = new Date(requiredOrder.createdAt);
    ...
    <body>
      <div class="container">
                   ...
                   <p>Dear <strong>${requiredOrder.user.name}</strong>,</p>
                   ...
                   <div class="order-details">
                       <p class="bold">Order Number: ${
                         requiredOrder.orderId
                       }</p>
                       <p class="bold">Order Date: ${orderDateIST}</p>
                       <p class="bold">Total Amount: ${
                         requiredOrder.grandTotal
                       }</p>
                   </div>
                   <div class="order-details">
    ...
    console.error("Error in SendOrderConfirmMailAfterSomething:", error);
    ...
    const order = await Order.aggregate([
      { $match: matchCondition },
      {
        $project: {
          orderId: 1,
          orderItems: {
            $sortArray: { input: "$orderItems", sortBy: { name: 1 } },
          }, // Sort orderItems alphabetically by name
          shippingInfo: 1,
          user: 1,
          paymentInfo: 1,
          paidAt: 1,
...
exports.updateOrderStatus = catchAsyncError(async (req, res, next) => {
  try {
    ...
    orderLogger.info(
      `Order status updated for Order ID - ${data[0].orderId} to ${req.body.status} for User ID - ${data[0].user.userId}`
    );
    res.status(200).json({
      success: true,
      data: result,
    ...
    orderLogger.info(
      `Order status updation failed for Order ID - ${data[0].orderId} to ${data[0].orderStatus} for User ID - ${data[0].user.userId}`
    );
  }
});
...
exports.getUserBillingInfoByOrderId = catchAsyncError(
  async (req, res, next) => {
    const orderId = req.params.orderId;
    if (!orderId) {
    ...
    try {
      const userBillingInfo = await Order.aggregate([
        ...
        },
        {
          $project: {
            ...
            address: {
              $ifNull: ["$shippingInfo.billingAddress.address", "N/A"],
            },
            locality: {
              $ifNull: ["$shippingInfo.billingAddress.locality", "N/A"],
            },
            landmark: {
              $ifNull: ["$shippingInfo.billingAddress.landmark", "N/A"],
            },
            city: { $ifNull: ["$shippingInfo.billingAddress.city", "N/A"] },
            pin_code: {
              $ifNull: ["$shippingInfo.billingAddress.pin_code", "N/A"],
            },
      ...
      console.error(
        "Error while getting user history of order",
        500,
        error.message
      );
...
exports.getUserPaymentDetailByOrderId = catchAsyncError(
  async (req, res, next) => {
    const orderId = req.params.orderId;
    if (!orderId) {
      throw new ErrorHandler("Order not found", 400);
...
exports.getUserDeliveryInfoByOrderId = catchAsyncError(
  async (req, res, next) => {
    const orderId = req.params.orderId;
    if (!orderId) {
    ...
    try {
      ...
      console.error(
        "Error while getting user delivery details of the order",
        500,
        error.message
      );
...
exports.updatePaymentStatusByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
...
exports.getQuantityWiseOrderByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
      const orderId = req.params.orderId;
      const page = parseInt(req.query.page) || 1;
...
exports.markOrderStatusPaidByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
      const orderId = req.params.orderId;
      ...
      throw new ErrorHandler(
        "Something wrong happend while updating order status"
      );
    }
  }
);

exports.markOrderStatusToCancelledByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
      const { orderId } = req.params;
      ...
      if (!cancelledOrder) {
        return res.status(404).json({
          success: false,
          message:
            "something wrong happed while updating order status to cancelled",
        });
      }
      const data = await Order.aggregate([
        {
      ...
      orderLogger.info(
        `Order status updated for Order ID - ${data[0].orderId} to canceled for User ID - ${data[0].user.userId}`
      );
      return res.status(200).json({
        success: true,
      ...
      console.error(error);
      throw new ErrorHandler(
        "Some thing wrong happed while updating order status"
      );
    }
  }
);

exports.getSingleOrderStatusByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
      const { orderId } = req.params;
...
exports.markOrderStatusAsDeliveredByOrderId = catchAsyncError(
  async (req, res, next) => {
    try {
      const { orderId } = req.params;
      ...
      orderLogger.info(
        `Order status updated for Order ID - ${data[0].orderId} to delivered for User ID - ${data[0].user.userId}`
      );
      return res.status(200).json({
        success: true,
...
exports.getOrderByOrderIdForUser = catchAsyncError(async (req, res, next) => {
  try {
              ...
              status: {
                $switch: {
                  branches: [
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /received/ },
                      },
                      then: "received",
                    },
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /accepted/ },
                      },
                      then: "accepted",
                    },
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /processing/ },
                      },
                      then: "processing",
                    },
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /packed/ },
                      },
                      then: "packed",
                    },
                    {
                    ...
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /transit/ },
                      },
                      then: "transit",
                    },
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /delivered/ },
                      },
                      then: "delivered",
                    },
                    {
                    ...
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /canceled/ },
                      },
                      then: "canceled",
                    },
                    {
                      case: {
                        $regexMatch: { input: "$message", regex: /failed/ },
                      },
                      then: "failed",
                    },
                  ],
    ...
    throw new ErrorHandler(
      "Some thing went wrong while getting order by order Id"
    );
  }
});

exports.updateDeliveryDetailsToOrder = catchAsyncError(
  async (req, res, next) => {
    try {
      const { orderId } = req.params;
      const { type, name, phone, email, _id } = req.body;
        ...
        { _id: orderId },
        {
          "deliveryInfo.deliveryType":
            type === "1"
              ...
              : "In house delivery partner" || null,
      ...
      console.error(error);
      throw new ErrorHandler(
        "Something went wrong while updating delivery partner details"
      );
    }
  }
);
            ...
            $slice: [
              {
                $map: {
                  ...
                  in: {
                    ...
                    incrementvalue: {
                      $ifNull: ["$$item.incrementvalue", "N/a"],
                    },
...
exports.updateOrderByAdmin = catchAsyncError(async (req, res, next) => {
  try {
    const { orderItems, paymentInfo, discountPrice, grandTotal } = req.body;
    const updatedData = {
      orderItems,
      paymentInfo,
      discountPrice,
...
exports.updateOrderStatusAfterPayment = catchAsyncError(
  async (req, res, next) => {
    const session = await mongoose.startSession();
