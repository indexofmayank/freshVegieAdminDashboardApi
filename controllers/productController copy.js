...
exports.updateProduct = catchAsyncError(async (req, res, next) => {
  ...
  let product = await Demoproduct.findById(req.params.id);
  if (!product) {
...
exports.getAllProductForTable = catchAsyncError(async (req, res, next) => {
  ...
  const products = await Demoproduct.aggregate([
    {
  ...
  const totalProducts = await Demoproduct.countDocuments();
...
exports.getProductByIdForAdmin = catchAsyncError (async (req, res, next) => {
  ...
  try {
    ...
    const product = await Demoproduct.aggregate([
      {
...
exports.getProductDetailByIdForUpdate = catchAsyncError (async (req, res, next) => {
  try {
    ...
    const product = await Demoproduct.findById({_id: productId});
    console.log(product);
